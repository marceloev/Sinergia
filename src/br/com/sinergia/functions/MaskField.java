...
public class MaskField {
    ...
    static Boolean IsAtualizando = true;
    ...
    public static void SpnFieldCtrl(Spinner<Integer> Spn, Integer Min, Integer Max) {
        ...
        Spn.getEditor().textProperty().addListener((obs, oldV, newV) -> {
            if (!newV.matches(oldV)) {
                Platform.runLater(() -> {
                    if (IsAtualizando) {
                        if (Spn.getEditor().getLength() == 0 || Integer.valueOf(Spn.getEditor().getText()) < Min) {
                            IsAtualizando = false;
        ...
        Spn.focusedProperty().addListener((obs, wasF, isF) -> {
            if (wasF) {
                ...
                Spn.getValueFactory().setValue(functions.getOnlyNumber(Spn.getEditor().getText()));
            }
        });
    }
